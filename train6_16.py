...
from model import TwoStageGenerator, Discriminator, UNet
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, segnet, mknet, mk_optim):
    ...
    for idx in pbar:
        ...
        mask = get_mask(mknet, segnet, fake_img)
        ...
        if get_rank() == 0:
            ...
            if i % 10000 == 0 and i != args.start_iter:
                torch.save(
                    ...
                    f"checkpoint/{str(i).zfill(6)}_6_16.pt",
                )

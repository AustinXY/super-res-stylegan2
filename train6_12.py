...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, segnet, mknet, mk_optim):
    ...
    for idx in pbar:
        ...
        if guide_regularize:
            ...
            mask1 = get_mask(mknet, segnet, img1)

            sep_loss = F.mse_loss(mask1 * j1, j1)
            ...
            mask2 = get_mask(mknet, segnet, img2)

            sep_loss += F.mse_loss(mask2 * j2, j2)
        ...
        if get_rank() == 0:
            ...
            if i % 10000 == 0 and i != args.start_iter:
                torch.save(
                    ...
                    f"checkpoint/{str(i).zfill(6)}_6_12.pt",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sep", type=float, default=2, help="mse weight")

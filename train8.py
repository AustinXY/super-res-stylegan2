...
from model import UNet, Generator, Discriminator, Encoder, Classifier
...
def train(args, loader, generator, discriminator, fine_generator, classifier, mpnet, g_optim, d_optim, mp_optim, cl_optim, g_ema, device):
    ...
    if args.distributed:
        ...
        cl_module = classifier.module
    else:
        g_module = generator
        d_module = discriminator
        fine_module = fine_generator
        mp_module = mpnet
        cl_module = classifier
    ...
    for idx in pbar:
        ...
        if guide_regularize:
            ...
            fake_img, _ = generator(torch.transpose(style_z, 0, 1).unbind(0), inject_nidex=args.injidx)

            preds = classifier(fake_img)

            b_info = F.cross_entropy(preds[0], torch.nonzero(b.long())[:, 1])
            p_info = F.cross_entropy(preds[1], torch.nonzero(p.long())[:, 1])
            c_info = F.cross_entropy(preds[2], torch.nonzero(c.long())[:, 1])

            z_dist = torch.mean(style_z)

            z_dist_ = z_dist * args.z_dist
            loss = (b_info + p_info + c_info) * args.info + z_dist_

            loss_dict["b_info"] = b_info
            loss_dict["p_info"] = p_info
            loss_dict["c_info"] = c_info
            loss_dict["z_dist"] = z_dist
        ...
        binfo_loss_val = loss_reduced["b_info"].mean().item()
        pinfo_loss_val = loss_reduced["p_info"].mean().item()
        cinfo_loss_val = loss_reduced["c_info"].mean().item()
        zdist_loss_val = loss_reduced["z_dist"].item()
        ...
        if get_rank() == 0:
                ...
                wandb.log(
                    {
                        ...
                        "B Info": binfo_loss_val,
                        "P Info": pinfo_loss_val,
                        "C Info": cinfo_loss_val,
                        "Z distribution": zdist_loss_val,
                    }
                )
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    fine_img = fine_generator(z, b, p, c)
                    style_z = mpnet(fine_img)
                    style_img, _ = g_ema(torch.transpose(style_z, 0, 1).unbind(0), inject_nidex=args.injidx)

                    fine_img1 = fine_generator(z, b, p, c1)
                    style_z = mpnet(fine_img1)
                    style_img1, _ = g_ema(torch.transpose(style_z, 0, 1).unbind(0), inject_nidex=args.injidx)
                ...
                torch.save(
                    {
                        ...
                        "cl_optim": cl_optim.state_dict(),
                        "mp_optim": mp_optim.state_dict(),
                        "args": args,
                        "ada_aug_p": ada_aug_p,
                        "cur_itr": i
                    },
                    f"checkpoint/{str(i).zfill(6)}_5_2.pt",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--z_dist", type=float, default=0.05, help="mse weight")
    ...
    train(args, loader, generator, discriminator, fine_generator, classifier, mpnet,

...
from model import Generator, Encoder, UNet
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        noise = mixing_noise(args.batch, args.latent, args.mixing, device)
        style_img, latent = style_generator(
    ...
    # style_img, latent = style_generator(noise, return_latents=True, randomize_noise=False)
    jc = torch.autograd.functional.jvp(lambda x: style_generator(
        x, input_is_latent=True, return_img_only=True), latent, v=v, create_graph=False, strict=False)
    ...
    svm = sn.heatmap(torch.sum(jc[1][0], dim=0).detach().cpu().numpy(), cmap='coolwarm')

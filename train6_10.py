    ...
    for idx in pbar:
        ...
        if guide_regularize:
            with torch.no_grad():
                ...
                _, ssc1 = generator(noise, inject_index=args.injidx, noise=img_noise, return_ssc=True)

                noise = mixing_noise(8, args.latent, args.mixing, device)
                _, ssc2 = generator(noise, inject_index=args.injidx, noise=img_noise, return_ssc=True)
                        ...
                        wandb.log(
                            {
                                ...
                                "mask1": [wandb.Image(Image.open(f"sample/{str(i).zfill(6)}_3.png").convert("RGB"))],
                            }
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enc", type=float, default=1, help="mse weight")
    parser.add_argument("--fg", type=float, default=10, help="mse weight")
    parser.add_argument("--bg", type=float, default=10, help="mse weight")
    parser.add_argument("--std", type=float, default=10, help="mse weight")

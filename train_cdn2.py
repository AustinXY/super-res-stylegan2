...
def train(args, fine_generator, style_generator, mpnet, decomposer, composer, dpr_optim, cpr_optim, device):
    ...
    for idx in pbar:
        ...
        loss_dict["vc"] = torch.zeros(1, device=device)
        loss_dict["ivc"] = torch.zeros(1, device=device)
        loss_dict["dpr"] = torch.zeros(1, device=device)
        ...
        fine_cpr_loss = F.mse_loss(rec_w, torch.ones_like(wp_code))
        ...
        # vc, ivc = decomposer(wp_code)
        # rec_w = composer(vc, ivc)
        # style_cpr_loss = F.mse_loss(rec_w, wp_code)
        style_cpr_loss = torch.zeros(1, device=device)
        loss_dict["s_cpr"] = style_cpr_loss

        loss_dict["cpr"] = fine_cpr_loss + style_cpr_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vc_dim", type=int, default=8092, help="variance code dim")
    parser.add_argument("--ivc_dim", type=int, default=100, help="invariance code dim")

# discriminator evaluate on 4 channel image (3 c image + 1 c mask)
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, segnet, mknet, mk_optim):
    ...
    for idx in pbar:
        ...
        real_mask = gaussian_noise(real_mask)
        real_img = torch.cat([real_img, real_mask], dim=1)
        ...
        #     mask = get_mask(segnet, style_img, unnorm=True)
        ...
        if get_rank() == 0:
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    style_img1, _ = g_ema(noise, inject_index=args.injidx, noise=img_noise)

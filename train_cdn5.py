...
from model import Generator, G_NET, Encoder, UNet, ImgCntMat
...
def train(args, fine_generator, style_generator, mpnet, mknet, cmnet, cm_optim, device):
    ...
    for idx in pbar:
        ...
        # ############# stage0: change bg #############
        # cmnet.requires_grad_(True)

        # noise = mixing_noise(args.batch, args.latent, args.mixing, device)
        # inj_idx = random.randint(1, style_generator.n_latent - 1)
        # style_img, orig_w = style_generator(
        #     noise, return_latents=True, randomize_noise=False, inject_index=inj_idx)
        # style_img = F.interpolate(style_img, size=(128, 128), mode='area')

        # w_mk = cmnet(orig_w)
        # bg_w_mk = torch.ones_like(w_mk) - w_mk

        # img_mk = mknet(style_img)
        # bg_img_mk = torch.ones_like(img_mk) - img_mk

        # fg = img_mk * style_img
        # bg = bg_img_mk * style_img

        # samples_fg = fg.view(args.batch, -1, 1)
        # samples_bg = bg.view(args.batch, -1, 1)

        # for _ in range(args.n_samples):
        #     noise = mixing_noise(args.batch, args.latent, args.mixing, device)
        #     _w = style_generator.get_mixed_latent(noise, inj_idx)
        #     w = orig_w * w_mk + _w * bg_w_mk

        #     style_img, _ = style_generator([w], input_is_latent=True, randomize_noise=False)
        #     style_img = F.interpolate(style_img, size=(128, 128), mode='area')

        #     fg = img_mk * style_img
        #     bg = bg_img_mk * style_img

        #     samples_fg = torch.cat((samples_fg, fg.view(args.batch, -1, 1)), dim=2)
        #     samples_bg = torch.cat((samples_bg, bg.view(args.batch, -1, 1)), dim=2)

        # fg_var = torch.var(samples_fg, dim=2).mean() * args.fg
        # bg_var = torch.var(samples_bg, dim=2).mean() * args.bg

        # stg0_loss = fg_var - bg_var

        # loss_dict["fg0"] = fg_var / args.fg
        # loss_dict["bg0"] = bg_var / args.bg
        ...
        style_img = F.interpolate(style_img, size=(128, 128), mode='area')
        img_mk = mknet(style_img)
        fg0 = img_mk * style_img

        w_mk = cmnet(style_img)
        bg_w_mk = torch.ones_like(w_mk) - w_mk
        ...
        style_img, _ = style_generator([w], input_is_latent=True, randomize_noise=False)
        style_img = F.interpolate(style_img, size=(128, 128), mode='area')
        fg1 = img_mk * style_img

        fg_var = F.mse_loss(fg0, fg1) * args.fg
        min_mk_loss = torch.mean(w_mk) * args.mmk

        stg0_loss = fg_var + min_mk_loss

        loss_dict["fg0"] = fg_var / args.fg
        loss_dict["mmk"] = min_mk_loss / args.mmk
        ...
        fg0_loss_val = loss_reduced["fg0"].mean().item()
        mmk_loss_val = loss_reduced["mmk"].mean().item()

        if get_rank() == 0:
            pbar.set_description(
                (
                    f"fg0: {fg0_loss_val:.4f}; mmk: {mmk_loss_val:.4f}"
                )
            )
            ...
            if i % 500 == 0:
                with torch.no_grad():
                    ...
                    inj_idx = random.randint(1, style_generator.n_latent - 1)
                    style_img0, orig_w = style_generator(
                        noise, return_latents=True, randomize_noise=False, inject_index=inj_idx)

                    _style_img0 = F.interpolate(style_img0, size=(128, 128), mode='area')
                    w_mk = cmnet(_style_img0)

                    bg_w_mk = torch.ones_like(w_mk) - w_mk

                    noise = mixing_noise(
                        args.n_sample, args.latent, args.mixing, device)
                    _w = style_generator.get_mixed_latent(noise, inj_idx)
                    w = orig_w * w_mk + _w * bg_w_mk

                    style_img1, _ = style_generator([w], input_is_latent=True, randomize_noise=False)
                    ...
                    if wandb and args.wandb:
                        wandb.log(
                            {
                                "image0": [wandb.Image(Image.open(f"sample/{str(i).zfill(6)}_0.png").convert("RGB"))],
                                "image1": [wandb.Image(Image.open(f"sample/{str(i).zfill(6)}_1.png").convert("RGB"))],
                            }
                        )
                ...
                torch.save(
                    {
                        ...
                        "cm": cm_module.state_dict(),
                        "cm_optim": cm_optim.state_dict(),
                        "args": args,
                        "cur_iter": i,
                    },
                    f"checkpoint/{str(i).zfill(6)}_5_.pt",
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fg", type=float, default=1e2, help="weight of the latent recon loss")
    parser.add_argument("--bg", type=float, default=1, help="weight of mask recon")
    parser.add_argument("--mmk", type=float, default=1e-1, help="weight of mask recon")
    ...
    if args.mp_ckpt is not None:
        print("load style model:", args.mp_ckpt)
        mp_dict = torch.load(args.mp_ckpt, map_location=lambda storage, loc: storage)
        style_generator.load_state_dict(mp_dict["style_g"])
        mknet.load_state_dict(mp_dict["mk"])
        # mpnet.load_state_dict(mp_dict["mp"])
    ...
    if get_rank() == 0 and wandb is not None and args.wandb:
        wandb.init(project="w mask")

    train(args, fine_generator, style_generator, mpnet, mknet, cmnet, cm_optim, device)

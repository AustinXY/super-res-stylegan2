...
def train(args, loader, fine_generator, style_generator, style_discriminator, mpnet, mp_optim, d_optim, device):
    ...
    for idx in pbar:
        ...
        style_img, _ = style_generator(noise, randomize_noise=False)
        _style_img = F.interpolate(style_img, size=(128, 128), mode='area')

        wp_code = mpnet(_style_img)

        recon_img, _ = style_generator([wp_code],
                                       input_is_latent=True,
                                       truncation=truncation,
                                       truncation_latent=trunc,
                                       randomize_noise=False)
        ...
        # adv loss
        fake_pred = style_discriminator(recon_img)
        adv_loss = g_nonsaturating_loss(fake_pred)
        loss_dict["adv"] = adv_loss

        # mse loss
        mse_loss = F.mse_loss(recon_img, style_img)
        ...
        if get_rank() == 0:
            ...
            if i % 20000 == 0 and i != args.start_iter:
                torch.save(
                    ...
                    f"checkpoint/{str(i).zfill(6)}_2.pt",
                )
...
if __name__ == "__main__":
    ...
    if get_rank() == 0 and wandb is not None and args.wandb:
        wandb.init(project="map net create syntax")
